...
assert rounded_val == 3.46, f"rounded_val should be 3.46, got {rounded_val}"
assert floored_val == 3, f"floored_val should be 3 (floor of 3.456), got {floored_val}"
assert isinstance(floored_val, int), (
    f"floored_val should be an int (math.floor returns int), got {type(floored_val).__name__}"
)
assert ceiled_val == 4, f"ceiled_val should be 4 (ceil of 3.456), got {ceiled_val}"
